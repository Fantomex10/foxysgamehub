...
const AppContent = () => {
    ...
    // Dynamically get the GameRoom component based on the game type
    const GameRoom = gameData?.gameType ? gameRegistry[gameData.gameType]?.GameComponent : null;
    ...
    const isInGame = !!gameData?.id && gameData.status !== 'waiting' && gameData.status !== 'lobby';
    ...
    return (
        <div className="min-h-screen w-full bg-gray-900 text-white font-sans flex flex-col">
            ...
            <MenuPanel isOpen={isMenuOpen} onClose={() => setIsMenuOpen(false)} onGoToLobby={handleGoToLobby} onOpenOptions={handleOpenOptions} onQuitGame={handleQuitGame} />
